...
from attacks.generate_backdoor_trigger import FinetuneTest
...
def parse_arguments():
    ...
    parser.add_argument('--trigger-train', type=bool, default=False, help='backdoor type')
    parser.add_argument('--trigger-pretrain', type=bool, default=False, help='backdoor type')
    ...
    parser.add_argument('--backdoor-nums', default=4, type=int, help='the backdoor nums')
    parser.add_argument('--eval-nums', default=20, type=int, help='sample nums for eval and test dataset')
    parser.add_argument('--pretrained', type=str, default=None, help='pretrained model path')
    parser.add_argument('--distance', type=str, default=None, help='pretrained model path')
...
if __name__ == '__main__':
    ...
    # 检查指定的 GPU 编号是否在有效范围内
    if args.gpu >= available_gpus:
        raise ValueError(f"Invalid GPU index {args.gpu}. Available GPUs: {available_gpus - 1}")
    else:
